...
(llm-tool-collection-deftool sed-edit-file
    (:category "filesystem" :tags (editing) :confirm t)
    ((file-path "File path to edit" :type string)
     (expression "sed expression, e.g., 's/foo/bar/g'" :type string))
    "Edit file using sed stream editor.

Examples:
- Replace: 's/old/new/g'
- Delete lines: '/pattern/d' or '5,10d'
- Insert: '/pattern/i\\text'
- Append: '/pattern/a\\text'"
  (let* ((path (expand-file-name file-path))
       (args (append
              (list "-i" "-e" expression path)))
       (temp-buffer (generate-new-buffer " *sed-output*"))
       (exit-code (apply #'call-process "sed" nil temp-buffer nil args)))
  (if (= 0 exit-code)
      (format "✅ Success: sed %s" (mapconcat #'shell-quote-argument args " "))
    (format "❌ Failed (exit %s)" exit-code))))
